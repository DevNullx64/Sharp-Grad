...
{
    public class MulOp<TFrom, TTo> : IApplyOpTwo<TFrom, TTo>
    ...
    public class MulOp<TFrom, TTo, TGrad> : MulOp<TFrom, TTo>, IBackwardTwo<TFrom, TTo, TGrad>
        where TFrom : unmanaged, INumber<TFrom>
        where TTo : unmanaged, INumber<TTo>
        where TGrad : unmanaged, IFloatingPoint<TGrad>
    {
        ...
        public static void Apply(Index1D idx,
            ArrayView1D<TFrom, Stride1D.Dense> left,
            ArrayView1D<TFrom, Stride1D.Dense> right,
            ArrayView1D<TTo, Stride1D.Dense> output)
            => output[idx] = ApplyCpu(left[idx], right[idx]);

        public static void BackwardAccelerator(Index1D idx,
        ...
        {
            var (l, r) = MulOp<TFrom, TTo, TGrad>.BackwardCpu(grad[idx], right[idx], left[idx]);
            leftGrad[idx] += l;
            rightGrad[idx] += r;
        }
    }
