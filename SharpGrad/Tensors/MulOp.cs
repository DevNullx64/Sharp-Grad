...
{
    public struct MulOp<TType> : IBackwardTwo<TType>
        where TType : unmanaged, IFloatingPoint<TType>
    {
        public static TType Apply(TType left, TType right) => left * right;
        public static (TType Left, TType Right) Backward(TType grad, TType left, TType right) => (grad * right, grad * left);
        ...
        {
            var (l, r) = MulOp<TType>.Backward(grad[idx], right[idx], left[idx]);
            leftGrad[idx] += l;
            rightGrad[idx] += r;
        }
    }
