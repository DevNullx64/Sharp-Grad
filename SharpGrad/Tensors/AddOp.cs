...
{
    public struct AddOp<TType> : IBackwardTwo<TType>
        where TType : unmanaged, IFloatingPoint<TType>
    {
        public static TType ApplyCpu(TType left, TType right) => left + right;
        public static (TType Left, TType Right) BackwardCpu(TType grad, TType left, TType right) => (grad, grad);
        ...
        {
            var (l, r) = AddOp<TType>.BackwardCpu(grad[idx], right[idx], left[idx]);
            leftGrad[idx] += l;
            rightGrad[idx] += r;
        }
    }
