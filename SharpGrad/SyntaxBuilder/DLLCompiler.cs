...
{{
    ...
    {{
        ...
        {{
            ...
            {{
                accumulator = O.{nameof(IBinaryOperation<float>.Operate)}(accumulator, input[{inputAccessString}]);
            }}
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void ReduceBackwards_{indicesInfo.MethodSuffix}<G>({parametersIndiceNames}, G[{arrayIndices}] inputGrad, G[{arrayIndices}] resultGrad)
            where G : {typeof(IFloatingPoint<>).GetName("G")}
        {{
            inputGrad[{inputAccessString}] += resultGrad[{resultAccessString}];
        }}
    }}
}}";
        }
